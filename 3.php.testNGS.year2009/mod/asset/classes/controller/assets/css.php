...
class Controller_Assets_CSS extends Controller_Assets
{
	...
	{   
		...
		else
		{
			...
			require_once SMVC::find_file('vendor', 'CSSAggregator');
	...
	protected function load_file($path)
	{
		...
		if ( ! $filename)
		{
			...
			throw new SMVC_Request_Exception('Unable to find CSS file: <tt>:path</tt>',
				array(':path' => $path));
		}
	...
	protected function compress($css, $config)
	{
		...
		{
			case 'strip':
				// Borrowed from the old SMVC media module:
				// http://code.google.com/p/SMVCmodules/source/browse/tags/2.2/media/controllers/media.php
			...
			default:
				throw new SMVC_Exception('Unknown CSS compression type :type',
